...
// sourceNames maps RB3Enhanced's raw origin symbol (as reported by the song
// list's game_origin field) to its display name, sourced from Rock Band 3
// Deluxe's source list localization (_ark/dx/locale/dx_locale_sources.dta) -
// plus rb1/rb2/rb3 (the base-game discs), which that file omits since DX has
// no need to label them in its source picker.
...
// sourceDisplayName returns the human-readable name for an origin symbol, or
// the symbol itself if it's not in sourceNames (e.g. a source DX hasn't
// localized, or a raw base-game origin).
func sourceDisplayName(raw string) string {
	if name, ok := sourceNames[raw]; ok {
		return name
	}
	return raw
}
