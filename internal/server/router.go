...
func NewRouter(hub *rb3net.Hub, database *sql.DB, staticDir, storeDir, templateDir string) http.Handler {
	...
	mux.Handle("/config/wled-devices/details", handleWLEDDeviceDetails(database))
	...
	mux.Handle("/config/wled-devices/colours", handleWLEDDeviceColours(database))
	mux.HandleFunc("/ws", handleWS(hub, storeArtDir, database))
